define([
    ...
    ko.components.register("api-form", {
        ...
        template: {require: "text!${ContextPathSlash}httplive/components/api-form.html"}
    });

    ko.components.register("modal", {
        viewModel: {require: "httplive/components/modal"},
        template: {require: "text!${ContextPathSlash}httplive/components/modal.html"}
    });
    ...
    function PageViewModel() {
        ...
        self.downloadFile = ko.computed(function () {
            return "${ContextPath}/httplive/webcli/api/downloadfile?id=" + this.selectedEndpointId();
        }, this);
        ...
        self.selectedFullPath = ko.computed(function () {
            if (this.selectedEndpoint()) {
                ...
                return origin + "${ContextPath}" + endpoint;
            }
        }, this);
        self.saving = ko.computed(function () {
            if (self.progress()) {
                ...
                return (
                    '<span class="span-status">Saving&nbsp;</span><img src="${ContextPath}/httplive/img/auto_saving.gif?' +
                    p +
                    '" />'
                );
            }
    ...
    webcli.subscribe(webcli.events.treeChanged, function (sender, context) {
        ...
        const id = context.id;
        const originKey = context.originKey;
        ...
        const url = config.fetchPath + "?id=" + id;
